...
def extrapolation(modeldict,
    ...
    for model_type, model_properties in modeldict.items():
        ...
        for name in landscape_dict.keys():
            ...
            distances = [d for d in distances if d] # Deletes zero if it listed as a distance
            results = []

            for i,instance in enumerate(landscape):

                instance_results = np.zeros((len(distances),len(distances),cross_validation))
                ...
                for fold in range(cross_validation):
                    ...
                    for d in distances:
                        ...
                        x_train, y_train, x_test, y_test = instance.sklearn_data(distance=d)
                    ...
                    for j, d in enumerate(distances):
                        ...
                        if this_model.__class__.__name__ == "NeuralNetRegressor":
                            ...
                            print("{0} trained on Dataset {1} distances 1-{2}".format(model_type,name,d))
                            print()
                            for k,test_dataset in enumerate(test_datasets):
                                score = this_model.score(test_dataset[0],test_dataset[1].reshape(-1,1))
                                print("On dataset {0}, fold {1}, for distance {2}, {3} returned an R-squared of {4}".format(name, fold, distances[k], model_type, score))
